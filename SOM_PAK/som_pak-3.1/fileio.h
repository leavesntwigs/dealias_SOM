...
char *som_pak_getline(struct file_info *fi);
/*
{
  ssize_t linelen;
  char line[2048];
  ssize_t linecap;
  linelen = getline((char **) &line, &linecap, fi->fp);
  if (linelen > 0)
    return line;
}
*/
...
char *som_pak_setprogname(char *argv0);
#define getprogname() som_pak_setprogname(NULL);
